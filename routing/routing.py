...
CORS(routing_bp)  # Enable CORS for all routes
...
@routing_bp.route('/search-stops', methods=['GET'])
def search_stops():
...
@routing_bp.route('/routes-multicriteria', methods=['POST'])
...
# Error handlers can be registered on the blueprint
@routing_bp.app_errorhandler(404)
def not_found(error):
    return jsonify({'error': 'Endpoint not found'}), 404


@routing_bp.app_errorhandler(500)
...
if __name__ == '__main__':
    # This block is for standalone running, which is now handled by app.py
    ...
    print(f"\n🟦 Routing backend running at: http://{local_ip}:5002\n")
